...
from src.controllers.pawpal_v2 import PawPalMQTTManager


DEVICE_ID = "pawpal-uuid12345678"
...
def publish_mock_recording(
    ...
    *,
    ...
    for seq in range(total_seq):
        ...
        client.publish(
            topic=PawPalMQTTManager.TOPIC_RECORDING.format(device_id=DEVICE_ID),
            payload=packet,
            qos=2 if seq + 1 == total_seq else 0,
        )


def subscribe_command(client: mqtt_client.Client, message_packer: MessagePacker):
    def on_message(client: mqtt_client.Client, userdata: Any, msg: mqtt_client.MQTTMessage):
        ...
        if server_cmd == "recording":
            publish_mock_recording(client=client, message_packer=message_packer)

    client.subscribe(PawPalMQTTManager.TOPIC_COMMAND.format(device_id=DEVICE_ID), qos=2)
    client.on_message = on_message


def subscribe_speaker(client: mqtt_client.Client, message_packer: MessagePacker):
    def on_message(client: mqtt_client.Client, userdata: Any, msg: mqtt_client.MQTTMessage):
        metadata, chunk = message_packer.unpack(msg.payload)
        print(f"Received SPEAKER {metadata!r} from {msg.topic!r} topic")

    client.subscribe(PawPalMQTTManager.TOPIC_SPEAKER.format(device_id=DEVICE_ID), qos=2)
    client.on_message = on_message
...
def main():
    ...
    subscribe_command(client, message_packer)
    subscribe_speaker(client, message_packer)
