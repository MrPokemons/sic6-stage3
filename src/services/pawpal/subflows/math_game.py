...
class MathGame(Agentic):
    ...
    @classmethod
    async def _start(cls, state: MGSessionState, config: ConfigSchema) -> Command[Literal["generate_question"]]:
        messages = [
            SystemMessage(
                content=[
                    {
    ...
    @classmethod
    async def _generate_question(cls, state: MGSessionState, config: ConfigSchema) -> Command[Literal["ask_question"]]:
        ...
        list_qna: List[MathQnA] = [
            MathQnA(
                sequence=(
                    MathQnA.generate_sequence(length=LENGTH, min_val=MIN_VAL, max_val=MAX_VAL)
                )
            )
    ...
    @classmethod
    async def _evaluate(cls, state: MGSessionState, config: ConfigSchema) -> Command[Literal["evaluate", "elaborate", "ask_question"]]:
        ...

    @classmethod
    async def _elaborate(cls, state: MGSessionState, config: ConfigSchema) -> Command[Literal["ask_question"]]:
        ...
    ...
    def build_workflow(self) -> CompiledStateGraph:
        ...
        builder.add_node("evaluate", self._evaluate)  # will contain eval, try again, congratz
